...
namespace MFCModernUI
{
    ...
    void CMListView::DrawHeader(CDC* pDC, const CRect& rect)
    {
        const ThemeColors& colors = GetColors();
        ...
        for (const auto& col : m_columns)
        {
            ...
            pDC->DrawText(col.header, colRect, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
            x += col.width;
        }
    ...
    void CMListView::DrawItems(CDC* pDC, const CRect& rect)
    {
        // 클리핑 영역 설정
        CRgn clipRgn;
    ...
    void CMListView::DrawItem(CDC* pDC, int index, const CRect& itemRect)
    {
        const ThemeColors& colors = GetColors();
        ...
        pDC->FillSolidRect(itemRect, bgColor);
        ...
        if (m_showCheckboxes)
        {
            ...
            if (item.checked)
            {
                pDC->FillSolidRect(checkRect, colors.primary.normal);

                // 체크마크
                CPen pen(PS_SOLID, 2, colors.textOnPrimary);
                CPen* pOldPen = pDC->SelectObject(&pen);
        ...
        for (size_t col = 0; col < m_columns.size() && col < item.texts.size(); col++)
        {
            ...
            pDC->DrawText(item.texts[col], textRect, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
